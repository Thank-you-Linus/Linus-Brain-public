...
class FeatureFlagManager:
    ...
    def filter_enabled_areas(self, area_ids: Iterable[str]) -> list[str]:
        ...
        enabled = [
            area_id for area_id in area_ids_list 
    ...
    async def evaluate_activity_if_enabled(
        ...
        if not self.is_area_enabled(area_id):
            self._metrics["skipped_evaluations"] += 1
            _LOGGER.info(f"FeatureFlag: Skipping activity evaluation for disabled area {area_id}")
            return None
        
        try:
            activity = await activity_tracker.async_evaluate_activity(area_id)
            _LOGGER.debug(f"FeatureFlag: Evaluated activity for enabled area {area_id}: {activity}")
            return activity
        except Exception as err:
            _LOGGER.error(f"FeatureFlag: Failed to evaluate activity for {area_id}: {err}")
            return None
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "skip_rate": self._metrics["skipped_evaluations"] / max(self._metrics["activity_evaluations"], 1),
        }
    ...
    async def validate_area_state(self, area_id: str) -> ValidationResult:
        ...
        if not is_enabled:
            ...
            suggestions.append("Enable the area in configuration to activate automations")
    ...
    def get_system_overview(self) -> dict[str, Any]:
        ...
        for area_id in enabled_areas:
            overview["area_details"][area_id] = self.get_area_status_explanation(area_id)
    ...
    def _assess_system_health(self) -> dict[str, Any]:
        ...
        checks["rule_engine"] = {
            ...
            "message": "Rule engine available" if rule_engine_available else "Rule engine not available",
        }
    ...
    def _generate_system_recommendations(self, overview: dict[str, Any]) -> list[str]:
        ...
        if metrics["enabled_areas_count"] == 0:
            recommendations.append("Enable areas in configuration to activate automations")
        
        if metrics["skip_rate"] > 0.3:
            recommendations.append("Review disabled areas - consider enabling if needed")
    ...
    def export_debug_data(self, format_type: str = "json") -> str:
        ...
        if format_type == "json":
            ...
            for area_id, details in overview["area_details"].items():
                lines.append(f"{area_id},{details['is_enabled']},{details.get('has_assignment', False)},\"{details['reason']}\"")
            return "\n".join(lines)
