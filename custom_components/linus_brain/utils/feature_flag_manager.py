...
class FeatureFlagManager:
    ...
    def is_feature_enabled(self, area_id: str, feature_id: str) -> bool:
        ...
        if is_enabled:
            ...
            _LOGGER.debug(f"FeatureFlag: Feature {feature_id} ENABLED for area {area_id}")
        else:
            self._metrics["disabled_checks"] += 1
            self._metrics["skipped_evaluations"] += 1
            _LOGGER.debug(f"FeatureFlag: Feature {feature_id} DISABLED for area {area_id}")
    ...
    def set_feature_enabled(self, area_id: str, feature_id: str, enabled: bool) -> None:
        ...
        if old_value != enabled:
            ...
            _LOGGER.info(f"FeatureFlag: Feature {feature_id} {'ENABLED' if enabled else 'DISABLED'} for area {area_id}")
    ...
    def get_all_feature_states(self) -> dict[str, dict[str, bool]]:
        ...
        """
        return {area: states.copy() for area, states in self._area_feature_states.items()}
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "skip_rate": self._metrics["skipped_evaluations"] / max(self._metrics["feature_evaluations"], 1),
        }
    ...
    def log_feature_status(self, area_id: str, feature_id: str, context: str = "") -> None:
        """
        ...
        if is_enabled:
            _LOGGER.info(f"FeatureFlag: Feature {feature_id} ENABLED for area {area_id} - Context: {context}")
        else:
            _LOGGER.info(f"FeatureFlag: Feature {feature_id} DISABLED for area {area_id} - Context: {context}")
        
        _LOGGER.debug(f"FeatureFlag: Feature status details: {log_data}")
    
    def get_feature_status_explanation(self, area_id: str, feature_id: str) -> dict[str, Any]:
        """
    ...
    async def validate_feature_state(self, area_id: str, feature_id: str) -> ValidationResult:
        """
        ...
        is_enabled = self.is_feature_enabled(area_id, feature_id)
        if not is_enabled:
            warnings.append(f"Feature '{feature_id}' is currently disabled for area '{area_id}'")
    ...
    def _assess_system_health(self) -> dict[str, Any]:
        ...
        checks["feature_definitions"] = {
            ...
            "message": f"{len(self._feature_definitions)} feature definitions available" if has_features else "No feature definitions available",
        }
    ...
    def _generate_system_recommendations(self, overview: dict[str, Any]) -> list[str]:
        ...
        if metrics["enabled_areas_count"] == 0:
            recommendations.append("Configure feature states for areas to enable app control")
        
        if metrics["skip_rate"] > 0.5:
            recommendations.append("Review disabled features - consider enabling if needed")
