...
def mock_entity_registry():
    ...
    mock_entities = [
        ...
        MockEntityEntry(
            "sensor.linus_brain_derniere_synchro",
            "linus_brain_last_sync",
            "last_sync"
        ),
        MockEntityEntry(
        ...
        MockEntityEntry(
            "sensor.linus_brain_erreurs",
            "linus_brain_errors",
            "errors"
        ),
        MockEntityEntry(
            "sensor.linus_brain_etat_cloud",
            "linus_brain_cloud_health",
            "cloud_health"
        ),
        MockEntityEntry(
        ...
        MockEntityEntry(
            "sensor.linus_brain_activites",
            "linus_brain_activities",
            "activities"
        ),
        ...
        MockEntityEntry(
            "button.linus_brain_synchroniser_maintenant",
            "linus_brain_sync",
            "sync"
        ),
...
async def test_migration_renames_french_entities(hass, mock_entity_registry):
    ...
    mock_entities = mock_entity_registry.async_entries_for_config_entry(mock_entry.entry_id)
    
    # Patch both er.async_get and er.async_entries_for_config_entry
    with patch("linus_brain.er.async_get", return_value=mock_entity_registry), \
         patch("linus_brain.er.async_entries_for_config_entry", return_value=mock_entities):
    ...
    for migration in migrations:
        ...
        assert expected_migrations[from_id] == to_id, f"Expected {from_id} → {expected_migrations[from_id]}, got {to_id}"
    
    # Verify we got all expected migrations
    assert len(migrations) == len(expected_migrations), f"Expected {len(expected_migrations)} migrations, got {len(migrations)}"
...
def test_suggested_object_id_in_sensors():
    ...
    from ..sensor import (
        ...
        LinusBrainCloudHealthSensor,
        LinusBrainRuleEngineStatsSensor,
        LinusBrainActivitiesSensor,
    )
...
def test_suggested_object_id_in_switch():
    ...
    with patch("linus_brain.switch.ar.async_get") as mock_ar:
        ...
        assert switch._attr_suggested_object_id == "linus_brain_feature_automatic_lighting_salon"
